...
def _rollout(model, initial_state, inputs, num_steps):
  ...
  _, _, output = tf.while_loop(
      ...
      body=step_fn,
      loop_vars=(0, initial_state['world_pos'],
...
def evaluate(model, inputs):
  ...
  prediction = _rollout(model, initial_state, inputs, num_steps)
